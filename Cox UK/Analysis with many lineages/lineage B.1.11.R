#--- //Do analysis for lineage B.1.11
...
metadtB_ <- coxukMeta %>% filter(lineage=="B.1.11") %>% select(sequence_name, sample_date)
nsamp_ <- 100 #--- //how many trees to sample?

#---- //sample nsamp_ trans. trees ----
#     * Store into a list
#----------------------------------------------------------------------//
ttSample_ <- sapply(names(lineage_), function(x) NULL)
for(nm_ in names(ttSample_)) {
  tmPhylo_<- getPhyloFromMetadata(lineage_[[nm_]], metadtB_)
...
rm(EMgamma, EMgammaND, extractCluster, gammamixSetup, getPhyloFromMetadata, getPhyloPerCluster, getSerialInterval, sampleTransTree, gammamixBoots)
#----------------------------------------------------------------------//
