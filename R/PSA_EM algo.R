#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Function to perform EM-algo with/without discretization
...
EMgammaND <- function(x, n.comp, alpha0=NULL, beta0=NULL, maxiter=1000, tol=1e-5, verbose=F) {
  #---//alpha and beta represent shape and scale parameters from Gamma distribution
  ...
  tau1 <- numeric(length(x))
  tau2 <- numeric(length(x))
  tau3 <- numeric(length(x))
  tau4 <- numeric(length(x))
  
  #--- //if we have x==0, we use discretization
  ...
  while(diff>tol & iter<maxiter & !(is.na(diff))) {
    ...
    #--- //get the relative probability for each data point x
  ...
  # notice that w1f1~Gamma(alpha, beta/w1) and for alpha/n.comp is bc there are n.comp components
